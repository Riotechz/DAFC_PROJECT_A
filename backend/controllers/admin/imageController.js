...
const findAllImage = async(req, res) => {
    try {
        ...
        if(dataToFind && dataToFind.isCountOnly){
            ...
            foundImage = await dbService.findMany( Image,query,options);
...
const getImageCount = async(req, res) => {
    try {
        ...
        if(!countedImage){
            ...
            countedImage = {totalRecords:countedImage}
            return res.success({data :countedImage});
...
const softDeleteManyImage = async (req, res) => {
    try{
        ...
        if(ids){
            ...
            const options={}
            let result = await dbService.softDeleteMany(Image,query,updateBody, options);
            if (!result) {
                return res.recordNotFound();
            }
...
const bulkUpdateImage=async(req, res)=>{
    try {
        ...
        if (dataToUpdate && dataToUpdate.data !== undefined) {
            ...
            let updatedImage = await dbService.updateMany(Image,filter,dataToUpdate);
            if(!updatedImage){
                return res.recordNotFound()
            }
...
const deleteManyImage =async(req, res) => {
    try{
        let dataToDelete = req.body;
            ...
            let deletedImage = await dbService.deleteMany(Image,query);
            return res.success({data :deletedImage});
    }
    catch(error){
        return res.internalServerError({message:error.message});  
    }
...
const partialUpdateImage = async (req, res) => {
    try {
        ...
        let updatedImage = await dbService.updateMany(Image, query, dataToUpdate);
        if (!updatedImage) {
            return res.recordNotFound();
        }
...
const getImage = async(req, res) => {
    try {
        ...
        let foundImage = await dbService.findByPk(Image,id,options);
        if(!foundImage){
            return res.recordNotFound();
        }
            return  res.success({data :foundImage});
